jQuery(document).ready(function () {
	...
	jQuery('table#orderList')
        .DataTable({
            ...
            columns: [
                ...
                {
                    'data': 'SupplierCountry.CountryName'
                },
                {
                    'data': 'SupplierNo'
                },
                ...
                },
                {
                    'data': 'EnableOnline',
                    "createdCell": function (td, cellData, rowData, row, col) {
                        jQuery(td).html(cellData ? "是" : ('<span style="color:red">' + "否" + '</span>'));
                    }
                },
                {
                    'data': 'SupplierSysName'
                },
                //状态
                {
                    'data': 'SupplierEnableState',
                    "createdCell": function (td, cellData, rowData, row, col) {
                ...
                //操作
                {
                    ...
                    "createdCell": function (td, cellData, rowData, row, col) {
                        var str =
                        '<div class="row">' +
                            '<a class="hrefInTable-inline" href="/Suppliers/Edit/' + cellData + '">修改</a>' +
    ...
    $('#operations').on("click", "a", function () {
        ...
        var SupplierID = arr.join(',');
        if (SupplierID.length == 0) {
            return;
        }
        $.ajax({
            url: "/Suppliers/UpdateDisable",
            type: 'post',
    ...
    $('body').on('click', '#exportproducts', function (e) {
        ...
        var varURL = "/Suppliers/ExportExcel?";
    ...
    jQuery("body").on('click', '#ResetPassWord', function (e, data) {
        ...
        var no = jQuery(this).closest("tr").find("td:eq(2)").text();
        var name = jQuery(this).closest("tr").find("td:eq(3)").text();

        jQuery.LangHua.confirm({
            title: "提示信息",
            tip1: '请确认是否为以下供应商重置密码：',
            tip2: no + "-" + name,
            confirmbutton: '确定',
            cancelbutton: '取消',
            data: null,
            confirm: function () {
                $.ajax({
                    url: "/Suppliers/ResetPassWord/" + id,
                    type: 'post',
                    dataType: 'json',
                    success: function (data) {
