...
{
    public class TBOrdersController : BaseController
    {
        ...
        public async Task<string> SaveTBOrder(TBOrderViewModel tBOrder)
        {
            ...
            bool IsCommit = tBOrder.IsCommit; //IsCommit为false时代表暂存
            ...
            try
            {
                ...
                foreach (var order in orders)
                {
                    ...
                    if (IsCommit)//正式提交时才验证以下内容
                    {
                        ...
                        if (TravelDate.Date < DateTime.Now.Date)//出行日期不能早于今天
                        {
                        ...
                        if (itemHistory.ServiceTypeID == 2)//行程
                        {
                        ...
                        if (itemHistory.ServiceTypeID == 4)//酒店类检查入住日期、退房日期和晚数
                        {
